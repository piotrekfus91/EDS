...
dojo.declare(
	...
	dijit.form._FormValueWidget,
	{
		...
		// delayOnChange: Integer
		//		Delay in milliseconds between when user types something and we 
		//		invoke onDelayedChange callback. When 0 no callback function
		//		will be invocked.
		delayOnChange: 0,
		
		// _timerOnChange: Object
		//		The timer which propagate delayed changes if delayOnChange > 0
		_timerOnChange: null,

		onDelayedChange: function(newValue){
			// summary:
			//		Callback when this widget's value is changed and specified
			//		delayOnChange>0 time exided.
			// tags:
			//		callback
		},
		...
		_onInput: function(e){
			...
			if(this.delayOnChange > 0) {
				...
				this._timerOnChange = setTimeout(function(){
					_this._timerOnChange = null;
					_this.onDelayedChange(_this.attr('value')); 
				}, this.delayOnChange);
			}
			this._refreshState();
		},
...
dojo.declare("dijit.form.Select", [dijit.form._FormSelectWidget, dijit._HasDropDown], {
	...
	_getMenuItemForOption: function(/*dijit.form.__SelectOption*/ option){
		...
		// CHANGE START - option value can be '' which is legal
		if(option.value == null){ //if(!option.value){
		// CHANGE END
	...
	openDropDown: function(){
		// summary:
		//		Opens the dropdown for this widget and set focus to selected
		//      item - it returns the return value of dijit.popup.open
		// tags:
		//		protected
        ...
        this.dropDown.focus = function() {
            var selected = dojo.filter(this.getChildren(), function(o) {
                return o.option && o.option.selected;
            })[0];
            if(selected) {
                this.focusChild(selected);
            } else {
                this.focusFirstChild();
            }
        }
...
dojo.i18n._preloadLocalizations("dijit.nls.dijit-form", ["ROOT","ar","ca","cs","da","de","de-de","el","en","en-gb","en-us","es","es-es","fi","fi-fi","fr","fr-fr","he","he-il","hu","it","it-it","ja","ja-jp","ko","ko-kr","nl","nl-nl","no","pl","pt","pt-br","pt-pt","ru","sk","sl","sv","th","tr","xx","zh","zh-cn","zh-tw"]);
