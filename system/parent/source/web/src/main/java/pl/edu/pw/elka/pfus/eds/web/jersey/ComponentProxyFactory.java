...
public class ComponentProxyFactory {
    ...
    public static Map<Class<?>, Object> proxies(PicoContainer container, ClassLoader cl) {
        ...
        for(ComponentAdapter adapter : (Collection<ComponentAdapter>)container.getComponentAdapters()) {
            ...
            if(interfaces.size() > 0) {
                Object proxy = Proxy.newProxyInstance(cl, interfaces.toArray(new Class[interfaces.size()]),
    ...
    private static Collection<Class<?>> getInheritedInterfaces(Collection<Class<?>> inheritedInterfaces,
                                                               Class<?> clazz) {
